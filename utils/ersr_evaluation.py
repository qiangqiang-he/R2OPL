...
By default, a checkpoint completes base evaluation and ERSR before the next
checkpoint is merged.  ``ersr.checkpoint_schedule: shared_teacher`` is an
opt-in throughput mode that instead batches Teacher replacements across
checkpoints and retains their temporary merged models until Student MC ends.
...
``student_action`` (correct trajectories, ``ERSR_Adv``) and/or
``teacher_replace`` (incorrect trajectories, ``ERSR_Adv_TR``); the latter
requires ``ersr.teacher`` or a matching entry in ``ersr.teachers`` for each
model family.  Checkpoint mode defaults to completing Base, Teacher, and
Student-MC phases for one checkpoint before loading the next, so merged
weights and rollout records do not accumulate in RAM or temporary storage.
An explicitly requested ``shared_teacher`` schedule is also available when
reloading a large Teacher is more expensive than that storage trade-off.
...
Launch with ``bash scripts/start_ersr_eval.sh configs/ersr_eval/CONFIG.yaml``.
"""
...
ERSR_ADV = "ERSR_Adv"
ERSR_ADV_TR = "ERSR_Adv_TR"
# Dedicated runs stay in a separate result tree even when a config omits it.
DEFAULT_RESULT_DIR = "./ersr_eval_results"
# The formal server contract is 16 samples per question and 64 samples per
# one-GPU vLLM call (four questions per full call).  Local smoke configs may
# override this with a smaller divisible value.
DEFAULT_ROLLOUTS_PER_GPU_BATCH = 64
RUN_NAME_PATTERN = re.compile(r"[A-Za-z0-9][A-Za-z0-9._-]*")


def summarize_ersr_metrics(
    ...
    *,
    ...
    """Report paper-facing ERSR metrics without changing the shared helper."""

    metric_names = {
        ERSR_STUDENT_ACTION: ERSR_ADV,
        ERSR_TEACHER_REPLACE: ERSR_ADV_TR,
    }
    selected: dict[tuple[str, str], int] = {}
    for case in selected_cases:
        key = (str(case.get("dataset", "")), str(case.get("action_type", "")))
        selected[key] = selected.get(key, 0) + 1

    values: dict[tuple[str, str], list[float]] = {}
    skipped: dict[tuple[str, str], int] = {}
    for result in results:
        key = (str(result.get("dataset", "")), str(result.get("action_type", "")))
        if bool(result.get("valid", False)):
            values.setdefault(key, []).append(float(result["advantage"]))
        else:
    ...
    for action_type in action_types:
        ...
        metric_name = metric_names[action_type]
        per_dataset: dict[str, dict[str, Any]] = {}
        all_values: list[float] = []
        dataset_means: list[float] = []
        total_selected = total_valid = total_skipped = 0
        for dataset in datasets:
            key = (str(dataset), action_type)
            ...
            selected_count = selected.get(key, len(dataset_values) + skipped.get(key, 0))
            skipped_count = skipped.get(key, 0)
            per_dataset[str(dataset)] = {
                "mean_advantage": mean,
                "selected_steps": selected_count,
                "valid_steps": len(dataset_values),
                "skipped_steps": skipped_count,
            }
            total_selected += selected_count
            total_valid += len(dataset_values)
        ...
        summary[metric_name] = {
            ...
            "macro_mean_advantage": (
                sum(dataset_means) / len(dataset_means)
                if dataset_means
                else None
            ),
            "weighted_mean_advantage": (
                sum(all_values) / len(all_values) if all_values else None
            ),
            "selected_steps": total_selected,
...
class ModelSpec:
    ...
    # Checkpoint evaluation can use the original base-model tokenizer while
    # loading merged checkpoint weights from a separate directory.
...
class ERSRConfig:
    ...
    # ``per_checkpoint`` is bounded-memory and follows the user-visible
    # checkpoint order. ``shared_teacher`` is an opt-in throughput mode.
    checkpoint_schedule: str = "per_checkpoint"
...
class EvaluationConfig:
    ...
    # Optional only for ``training_output`` mode.  Besides documenting the
    # Student's origin, it supplies tokenizer/config metadata when FSDP
...
class CheckpointSpec:
    ...
    model_dir: Path | None
    # True when ``prepare_model`` must stage ``base_model.path`` as the
    # FSDP merger's required ``huggingface`` metadata directory.
    uses_base_model_metadata: bool = False
...
def _parse_ersr(
    ...
    *,
    ...
    settings_input = {"max_steps_per_dataset": 200, **raw_ersr}
    student_only_teacher_budget: int | None = None
    # The shared training-time parser predates student-only evaluation and
    # insists on a positive teacher budget.  A teacher is never invoked for
    # this action set, so keep the compatibility value private to this new
    # evaluator instead of changing the existing utility.
    if ERSR_TEACHER_REPLACE not in actions:
        student_only_teacher_budget = int(
            settings_input.get("max_teacher_step_tokens", 0)
        )
        settings_input["max_teacher_step_tokens"] = max(
            1, student_only_teacher_budget
        )
    settings = parse_ersr_settings(
        settings_input,
        max_response_tokens=max_new_tokens,
    )
    if student_only_teacher_budget is not None:
        settings = replace(
            settings, max_teacher_step_tokens=student_only_teacher_budget
        )
    validate_ersr_dataset_subset(settings.datasets, val_datasets)

    teacher: ModelSpec | None = None
    teachers: list[ModelSpec] = []
    checkpoint_schedule = raw_ersr.get("checkpoint_schedule", "per_checkpoint")
    if (
        not isinstance(checkpoint_schedule, str)
        or checkpoint_schedule not in {"per_checkpoint", "shared_teacher"}
    ):
        raise ValueError(
            "ersr.checkpoint_schedule must be 'per_checkpoint' or "
            "'shared_teacher'."
        )
    if (
        checkpoint_schedule == "shared_teacher"
        and ERSR_TEACHER_REPLACE not in actions
    ):
        raise ValueError(
            "ersr.checkpoint_schedule=shared_teacher requires "
            "ersr.actions to include teacher_replace."
        )
...
def load_evaluation_config(
    ...
    *,
    ...
    if "training_output" in raw:
        ...
        if raw.get("base_model") is not None:
            base_model = _parse_model_entry(
                _mapping(raw["base_model"], "base_model"),
                0,
                label="base_model",
                project_root=project_root,
                default_family=default_family,
            ...
            if (
                ...
                and base_model.family != default_family
            ):
                raise ValueError(
    ...
    if rollouts_per_gpu_batch > DEFAULT_ROLLOUTS_PER_GPU_BATCH:
        raise ValueError(
            "runtime.rollouts_per_gpu_batch must be at most "
            f"{DEFAULT_ROLLOUTS_PER_GPU_BATCH} for dedicated ERSR evaluation."
        )
...
def _has_fsdp_huggingface_metadata(path: Path) -> bool:
    return (path / "huggingface" / "config.json").is_file()


def _validate_fsdp_source(path: Path, *, base_model: ModelSpec | None) -> None:
    config_path = path / "fsdp_config.json"
    if not config_path.is_file():
        raise ValueError(f"Incomplete FSDP actor metadata: missing {config_path}")
    if not _has_fsdp_huggingface_metadata(path) and base_model is None:
        raise ValueError(
            f"FSDP actor {path} does not contain huggingface/config.json. "
            "Set checkpoint-mode base_model (a complete Hugging Face model) "
            "to supply merger/tokenizer metadata."
        )
    try:
        with config_path.open(encoding="utf-8") as handle:
            world_size = _integer(
...
def _choose_fsdp_source(
    checkpoint_dir: Path, *, base_model: ModelSpec | None
) -> Path:
    candidates: list[Path] = []
    ...
    _validate_fsdp_source(source, base_model=base_model)
    return source
        ...
        if hf_model is not None:
            specs.append(
                CheckpointSpec(
                    ...
                    metadata_dir=hf_model,
                    model_dir=hf_model,
                )
            )
            continue
        fsdp_source = _choose_fsdp_source(
            checkpoint_dir, base_model=config.base_model
        )
        uses_base_model_metadata = not _has_fsdp_huggingface_metadata(fsdp_source)
        metadata_dir = (
            fsdp_source / "huggingface"
            if not uses_base_model_metadata
            else None
        )
        specs.append(
            CheckpointSpec(
                ...
                tokenizer_dir=(
                    config.base_model.path
                    if config.base_model is not None
                    else fsdp_source / "huggingface"
                ),
                metadata_dir=metadata_dir,
                model_dir=None,
                uses_base_model_metadata=uses_base_model_metadata,
            )
        )
    return specs


def _stage_fsdp_source_with_base_metadata(
    *,
    checkpoint: CheckpointSpec,
    base_model: ModelSpec,
    staging_dir: Path,
) -> Path:
    """Stage lightweight symlinks for a shard-only FSDP checkpoint.

    VERL's merger expects ``<local_dir>/huggingface`` next to the shards.
    Training outputs normally retain that directory, but a shard-only export
    can safely borrow the configured base model without copying multi-GB
    weights or mutating the training output.
    """

    staging_dir.mkdir(parents=True, exist_ok=False)
    try:
        for source in checkpoint.source_dir.iterdir():
            if (
                source.name in {"fsdp_config.json", "lora_train_meta.json"}
                or re.fullmatch(r"model_world_size_\d+_rank_\d+\.pt", source.name)
            ):
        ...
        raise RuntimeError(
            "Could not stage base-model metadata for FSDP checkpoint "
            f"{checkpoint.checkpoint_dir}: {exc}"
        ) from exc
    return staging_dir
...
def prepare_model(
    ...
    try:
        ...
        merge_source = checkpoint.source_dir
        if checkpoint.uses_base_model_metadata:
            if config.base_model is None:
                raise RuntimeError(
                    "Checkpoint requires base-model metadata but no base_model is configured."
                )
            merge_source = _stage_fsdp_source_with_base_metadata(
                checkpoint=checkpoint,
                base_model=config.base_model,
                staging_dir=temporary_root / "fsdp_source",
            )
        command = [
            sys.executable,
            "-m",
        ...
        print(
            ...
            f"metadata={'base_model' if checkpoint.uses_base_model_metadata else 'checkpoint'}",
            flush=True,
        )
        ...
        try:
            ...
            print(
                f"MERGED_MODEL_CLEANUP_FAILED step={checkpoint.step_num} "
                f"path={temporary_root} error={cleanup_error!r}",
                flush=True,
            )
            # If an earlier evaluation error is already propagating, retain it
            # as the primary failure while still making the residual directory
            # explicit in the log.  Otherwise surface cleanup failure: keeping
            # a full merged model silently can exhaust the checkpoint volume.
            if sys.exc_info()[0] is None:
                raise RuntimeError(
                    f"Could not remove merged checkpoint directory: {temporary_root}"
                ) from cleanup_error
        else:
            print(
...
class GpuPool:
    ...
    def __init__(
        ...
        *,
        ...
        self.tokenizer_dir = str(tokenizer_dir or model_dir)
    ...
    def __enter__(self) -> "GpuPool":
        print(
            f"GPU_POOL_START model={self.model_dir} tokenizer={self.tokenizer_dir} "
            f"gpus={list(self.config.gpus)}",
            flush=True,
        )
        started_ok = False
...
def evaluate_model(
    *,
    ...
    with GpuPool(
        ...
        family=model.family,
        tokenizer_dir=model.tokenizer_path,
...
def _ersr_settings_document(config: EvaluationConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "checkpoint_schedule": config.ersr.checkpoint_schedule,
        ...
        for work, case, request in rows:
            try:
                ...
                work.invalid[case["case_id"]] = {
                    ...
                    "action_type": case["action_type"],
                    "valid": False,
                    "skip_reason": "teacher_split_error",
                    "error": repr(exc),
                }
                continue
            if not replacement:
...
def evaluate_ersr_student(config: EvaluationConfig, work: ERSRWork) -> dict[str, Any]:
    ...
    document.update({
        "results": results,
        # Backward-compatible chart labels retained for existing analysis.
        "means": means,
        "skipped": skipped,
        # Canonical paper-facing ERSR_Adv / ERSR_Adv_TR views.  Unlike
        # ``means``, unavailable series are represented as null with counts.
        "metrics": summarize_ersr_metrics(
            results,
            settings.datasets,
            config.ersr.actions,
    ...
    return {
        ...
        "base_model": (
            ...
            if config.base_model is None
                ...
                "chat_template_path": (
                    None
                    if config.base_model.chat_template_path is None
                    else str(config.base_model.chat_template_path)
                ),
            }
        ),
        "seed": config.seed,
...
def build_validation_plan(
        ...
        plan["checkpoints"] = [
            {
                ...
                "tokenizer_dir": str(checkpoint.tokenizer_dir),
                "metadata_source": (
                    "base_model"
                    if checkpoint.uses_base_model_metadata
                    else "checkpoint"
    ...
    if config.ersr is not None:
        ...
        plan["ersr"]["schedule"] = (
            config.ersr.checkpoint_schedule
            if config.source_type == "checkpoints"
            else "per_model"
        )
    return plan
...
def _tokenizer_identity(
    tokenizer: Any,
) -> tuple[dict[str, int], tuple[int, ...], tuple[Any, Any, Any]]:
    """Return the token-ID mapping and special IDs relevant to raw handoff."""
...
def validate_tokenizers(config: EvaluationConfig, models: Sequence[ModelSpec]) -> None:
    ...
    teacher_tokenizers: dict[
        Path, tuple[dict[str, int], tuple[int, ...], tuple[Any, Any, Any]]
    ] = {}
    for model in models:
        tokenizer = load_prompt_tokenizer(config, model)
        student_identity = _tokenizer_identity(tokenizer)
        teacher = ersr_teacher_for_model(config, model)
        if teacher is None:
            continue
        if teacher.path not in teacher_tokenizers:
            validate_model_context(teacher.path, config.max_model_len)
            teacher_tokenizers[teacher.path] = _tokenizer_identity(
                _load_driver_tokenizer(config, teacher.path)
            )
        if student_identity != teacher_tokenizers[teacher.path]:
            raise ValueError(
                f"ERSR teacher {teacher.name} and student {model.name} must have identical "
                "token-to-ID vocabularies, special-token IDs, and BOS/EOS/PAD "
                "IDs for token-exact replacement."
            )


def validate_checkpoint_base_model(
    config: EvaluationConfig, checkpoints: Sequence[CheckpointSpec]
) -> None:
    """Verify an explicit base model against any checkpoint metadata.

    The base model is the tokenizer source for checkpoint evaluation.  Normal
    VERL FSDP checkpoints still contain their own ``actor/huggingface`` tree;
    compare it eagerly so a mismatched base path cannot silently turn prompt
    token IDs into different Student inputs.  Shard-only checkpoints have no
    metadata to compare and are handled by ``prepare_model``'s symlink stage.
    """
    ...
    base_tokenizer_identity: (
        tuple[dict[str, int], tuple[int, ...], tuple[Any, Any, Any]] | None
    ) = None
    if base_model is not None:
        validate_model_context(base_model.path, config.max_model_len)
        base_tokenizer_identity = _tokenizer_identity(
            _load_driver_tokenizer(config, base_model.path)
        )

    checked_metadata: set[Path] = set()
    for checkpoint in checkpoints:
        metadata_dir = checkpoint.metadata_dir
        if metadata_dir is None or metadata_dir in checked_metadata:
            continue
        checked_metadata.add(metadata_dir)
        # ``tokenizer_dir`` may be the explicitly configured base model.  The
        # checkpoint's own config can still impose a lower position limit, so
        # validate it before an expensive FSDP merge.
        validate_model_context(metadata_dir, config.max_model_len)
        actual_family = resolve_model_family(metadata_dir)
        if actual_family != expected_family:
            raise ValueError(
                f"Checkpoint {checkpoint.checkpoint_dir} has {actual_family} "
                f"metadata, but the configured Student family is {expected_family}."
            )
        if base_tokenizer_identity is not None:
...
def _evaluate_base_model(
    *,
    ...
    result: dict[str, Any] = {
        ...
        "tokenizer_path": str(model.tokenizer_path or model.path),
        "chat_template_path": None if model.chat_template_path is None else str(model.chat_template_path),
...
def _annotate_checkpoint_result(
    result: dict[str, Any], checkpoint: CheckpointSpec
) -> None:
    """Replace transient merged paths with permanent checkpoint provenance."""
    ...
    result.update(
        ...
        source_kind=checkpoint.source_kind,
        source_dir=str(checkpoint.source_dir),
        # The FSDP merged model directory is temporary.  Even when no explicit
        # base model is configured, tokenizer_dir is an actor/huggingface tree
        # that remains valid after the merge cache is removed.
        tokenizer_path=str(checkpoint.tokenizer_dir),
        checkpoint_metadata_source=(
            "base_model" if checkpoint.uses_base_model_metadata else "checkpoint"
        ),
    )
    if checkpoint.source_kind == "fsdp":
        # ``prepare_model`` removes its merged HF directory after this
        # checkpoint.  Never leave that dead path as the result's model path.
        result["model_path"] = str(checkpoint.source_dir)
        result["model_path_kind"] = "fsdp_actor_shards_merged_on_demand"
    else:
        result["model_path"] = str(checkpoint.source_dir)
...
def _checkpoint_model_for_directory(
    ...
    return _resolve_checkpoint_model(
        checkpoint,
        model_dir,
        base_model=config.base_model,
        model_family=config.model_family,
...
def _checkpoint_run_signature(config: EvaluationConfig) -> dict[str, Any]:
    """Stable inputs that must not change when resuming one checkpoint run."""

    metadata = _evaluation_metadata(config)
    return {
        ...
        "length_control": metadata["length_control"],
        "runtime": metadata["runtime"],
        "engine": metadata["engine"],
        "report": {"save_rollouts": config.save_rollouts},
        "ersr": (
            None if config.ersr is None else _ersr_settings_document(config)
        ),
    }
...
def _load_checkpoint_document(
    ...
    *,
    ...
    """Create a result document or validate a resumable per-checkpoint run."""

    signature = _checkpoint_run_signature(config)
    if not config.result_path.exists():
        return {
            "run_name": config.run_name,
            "training_output": str(config.training_output),
            **_evaluation_metadata(config),
            "checkpoint_run_signature": signature,
            "status": "running",
            "phase": "base",
            "started_at": utc_now(),
            "steps": [],
    ...
    if not resume:
        raise FileExistsError(
            f"Result file already exists: {config.result_path}; choose a new "
            "run_name or pass --resume for a per_checkpoint run."
        )
    try:
        with config.result_path.open(encoding="utf-8") as handle:
            document = json.load(handle)
    except (OSError, json.JSONDecodeError) as exc:
        raise ValueError(
            f"Cannot resume unreadable result file {config.result_path}: {exc}"
        ) from exc
    if not isinstance(document, dict):
        raise ValueError(f"Cannot resume malformed result file: {config.result_path}")
    if document.get("checkpoint_run_signature") != signature:
        raise ValueError(
            "Refusing to resume because the evaluation configuration or model "
            "paths differ from the existing result file. Use a new run_name."
        )
    steps = document.get("steps")
    if not isinstance(steps, list):
        raise ValueError("Cannot resume result file with a non-list steps field.")
    seen_steps: set[int] = set()
    for step in steps:
        if not isinstance(step, Mapping) or not isinstance(step.get("step_num"), int):
            raise ValueError("Cannot resume result file with malformed step entries.")
        step_num = int(step["step_num"])
        if step_num in seen_steps:
            raise ValueError("Cannot resume result file with duplicate checkpoint steps.")
    ...
    if document.get("status") == "completed":
        ...
        if completed_steps != set(expected_steps):
            raise ValueError(
...
def _run_checkpoint_per_checkpoint(
    *,
    ...
    for checkpoint in checkpoints:
        if checkpoint.step_num in completed_steps:
            print(f"STEP_SKIP_COMPLETED step={checkpoint.step_num}", flush=True)
            continue
        stale = [
            result
            for result in document["steps"]
            if int(result.get("step_num", -1)) == checkpoint.step_num
        ]
        if stale:
            # An incomplete step has no retained base rollout records, so it
            # is safely restarted from Base instead of mixing attempts.
            document["steps"] = [
                result
                for result in document["steps"]
                if int(result.get("step_num", -1)) != checkpoint.step_num
            ]
        document["phase"] = f"global_step_{checkpoint.step_num}:base"
        atomic_write_json(config.result_path, document)
        with prepare_model(config, checkpoint) as model_dir:
            model = _checkpoint_model_for_directory(config, checkpoint, model_dir)
            print(
                f"STEP_BASE_START step={checkpoint.step_num} "
                f"source={checkpoint.source_kind}",
                flush=True,
            )
            result, tokenizer, records = _evaluate_base_model(
                config=config,
            ...
            if config.ersr is not None:
                ...
                document["phase"] = f"global_step_{checkpoint.step_num}:teacher"
                atomic_write_json(config.result_path, document)
                generate_ersr_replacements(config, [work])
                # Persist teacher proposals before Student MC, so a failure is
                # still fully auditable even though this attempt will restart
                # the incomplete checkpoint from base on the next invocation.
                result["ersr"] = {
                    **work.document,
                ...
                document["phase"] = f"global_step_{checkpoint.step_num}:student"
                atomic_write_json(config.result_path, document)
                print(
            ...
            result.update(status="completed", completed_at=utc_now())
            atomic_write_json(config.result_path, document)
            print(f"STEP_DONE step={checkpoint.step_num}", flush=True)
            del records, tokenizer
            if config.ersr is not None:
                del work


def _run_checkpoint_shared_teacher(
    *,
    config: EvaluationConfig,
    checkpoints: Sequence[CheckpointSpec],
    questions: Sequence[Mapping[str, Any]],
...
def evaluate_checkpoints(
    ...
    *,
    ...
    use_shared_teacher = (
        config.ersr is not None
        and config.ersr.checkpoint_schedule == "shared_teacher"
    )
    if use_shared_teacher and resume:
        raise ValueError(
            "--resume is only supported by ersr.checkpoint_schedule=per_checkpoint; "
            "shared_teacher retains cross-checkpoint in-memory work."
        )
    if resume and not config.result_path.exists():
        raise FileNotFoundError(
            f"--resume requires an existing result file: {config.result_path}"
        )
    document, resumed = _load_checkpoint_document(
        config,
        resume=resume,
        expected_steps=[checkpoint.step_num for checkpoint in checkpoints],
    )
    if use_shared_teacher and resumed:
        raise ValueError(
            "shared_teacher checkpoint evaluation cannot resume; use a new run_name."
        )
    if document.get("status") == "completed":
        print(f"EVALUATION_ALREADY_COMPLETED result={config.result_path}", flush=True)
        return
    atomic_write_json(config.result_path, document)
    try:
        if use_shared_teacher:
            _run_checkpoint_shared_teacher(
                config=config,
            ...
            _run_checkpoint_per_checkpoint(
                config=config,
                checkpoints=checkpoints,
                questions=questions,
                question_counts=question_counts,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Restart incomplete checkpoints in a failed/running checkpoint evaluation. "
            "Supported only for ersr.checkpoint_schedule=per_checkpoint."
        ),
    )
    return parser.parse_args(argv)


def _resolve_checkpoint_model(
    checkpoint: CheckpointSpec,
    model_dir: Path,
    *,
    base_model: ModelSpec | None = None,
    ...
    """Attach permanent tokenizer/family metadata to transient checkpoint weights."""

    if base_model is not None:
        family = base_model.family
        chat_template_path = base_model.chat_template_path
        tokenizer_path: Path | None = base_model.path
    else:
        family = model_family or resolve_model_family(
            model_dir,
            explicit=(
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if config.source_type == "checkpoints":
        validate_checkpoint_base_model(config, checkpoints)
        models_for_tokenizer_validation = [
            _resolve_checkpoint_model(
                checkpoint,
                checkpoint.tokenizer_dir,
                base_model=config.base_model,
                model_family=config.model_family,
            )
            for checkpoint in checkpoints
        ...
        models_for_tokenizer_validation = list(config.models)
    validate_tokenizers(config, models_for_tokenizer_validation)
    ...
    if config.source_type == "models":
        if args.resume:
            raise ValueError(
                "--resume currently supports training_output checkpoint evaluation only."
            )
