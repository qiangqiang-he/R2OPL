...
def _torch_save_via_local_temp(state_dict, destination: str) -> None:
    """Serialize locally, then publish to shared storage with an atomic rename.

    PyTorch's zip writer performs seeks that some shared filesystems can report
    inconsistently for multi-GB concurrent writes.  A node-local staging file
    keeps serialization on a regular filesystem; the destination receives only
    a sequential copy.
    """

    destination = os.path.abspath(destination)
    destination_dir = os.path.dirname(destination)
    staged_destination = f"{destination}.tmp-{os.getpid()}"
    local_path = None
    try:
        with tempfile.NamedTemporaryFile(prefix="verl-checkpoint-", suffix=".pt", delete=False) as local_file:
            local_path = local_file.name
        torch.save(state_dict, local_path)
        shutil.copyfile(local_path, staged_destination)
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
            if self.should_load_model:
                ...
                # Ray commonly exposes exactly one physical GPU to each worker,
                # remapped as cuda:0. A checkpoint resharded by torchrun may
                # retain cuda:<global-rank> storage tags, so always restore its
                # tensors onto this worker's current local CUDA device.
                model_state_dict = torch.load(
                    local_model_path,
                    map_location=f"cuda:{torch.cuda.current_device()}" if is_cuda_available else "cpu",
                    weights_only=False,
                )
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                if self.should_save_model:
                    ...
                    _torch_save_via_local_temp(model_state_dict, model_path)
                    log_with_rank(f"Saved model to {os.path.abspath(model_path)}", rank=self.rank, logger=logger)

                if self.should_save_optimizer:
                    optimizer_state_dict = self.optimizer.state_dict()
                    _torch_save_via_local_temp(optimizer_state_dict, optim_path)
                ...
                if self.should_save_extra:
                    ...
                    _torch_save_via_local_temp(extra_state_dict, extra_path)
