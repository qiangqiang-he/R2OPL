...
class RolloutConfig(BaseConfig):
    ...
    # R^2OPL targets server drivers that do not support vLLM sleep mode.
    # Keeping cache release disabled also prevents indirect sleep()/wake_up().
    free_cache_engine: bool = False
    data_parallel_size: int = 1
    expert_parallel_size: int = 1
    # One complete model per GPU is the default. Larger models must opt into
    # tensor parallelism explicitly instead of silently paying TP overhead.
