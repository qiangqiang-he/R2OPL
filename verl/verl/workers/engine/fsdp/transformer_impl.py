...
class FSDPEngineWithLMHead(FSDPEngine):
        ...
        if use_fused_kernels:
            ...
            if direct_chunked or combined_topk:
                ...
                extra_args.update(
                    ...
                    distillation_chunk_size=int(
                        tu.get_non_tensor_data(data=micro_batch, key="distillation_chunk_size", default=1024)
                    ),
                    distillation_log_prob_min_clamp=tu.get_non_tensor_data(
    ...
    def prepare_model_outputs(self, output, output_args, micro_batch: TensorDict, logits_processor_func):
        ...
        # ``distillation_direct_chunked`` (R2OPL legacy key) and ``distillation_only``
        # (upstream key) carry the same semantics; honor either assignment site.
        distillation_only = bool(
            tu.get_non_tensor_data(data=micro_batch, key="distillation_only", default=False)
        ) or bool(
            tu.get_non_tensor_data(
                data=micro_batch, key="distillation_direct_chunked", default=False
            )
