...
from verl.trainer.ppo.utils import Role, WorkerType, create_rl_dataset, create_rl_sampler, need_critic, need_reference_policy, need_teacher_policy
...
class AgentLoopWorkerTQ(AgentLoopWorker):
    ...
    async def generate_sequences(self, batch: TensorDict) -> None:
        ...
        # The rollout engine is provisioned for the larger validation context,
        # but training and validation use different generation limits.  This
        # must run inside the Ray worker (rather than as a driver-side monkey
        # patch), otherwise Ray reconstructs the original class remotely and
        # training silently inherits the validation limit.
        generation_config = self.config.get("rlvr_generation")
        if generation_config is not None:
            limit_key = "val_max_new_tokens" if validate else "train_max_new_tokens"
            sampling_params["max_tokens"] = int(generation_config[limit_key])
    ...
    async def evaluate_ersr_cases(
        self,
        cases: list[dict[str, Any]],
        settings_dict: dict[str, Any],
    ) -> list[dict[str, Any]]:
        """Evaluate ERSR cases with the already-loaded Student and Teacher."""

        from utils.ersr import ERSRSettings, evaluate_ersr_case

        settings = ERSRSettings(**settings_dict)

        async def student_generate(*, prompt_ids, max_tokens, seed):
            if int(max_tokens) <= 0:
                return []
            ...
            # Empty output is a valid continuation.  In particular, a Teacher
            # replacement may already contain the complete final answer.
            return [int(value) for value in output.token_ids]

        teacher_generate = None
        teacher_server_manager = getattr(self, "teacher_server_manager", None)
        if teacher_server_manager is not None:

            async def teacher_generate(*, prompt_ids, max_tokens, seed):
                return await teacher_server_manager.generate_replacement_step_tokens(
                    prompt_ids=[int(value) for value in prompt_ids],
                    max_tokens=int(max_tokens),
                    temperature=settings.temperature,
                    top_p=settings.top_p,
        ...
        for case in cases:
            try:
                ...
                # One malformed or backend-aborted intervention must not stop a
                # long training run.  Keep it auditable as an unavailable ERSR
                # case and continue with the remaining selected steps.
                logger.warning("ERSR case %s failed: %s", case.get("case_id"), exc)
                results.append(
                    {
                        "case_id": str(case.get("case_id", "unknown")),
                        "dataset": str(case.get("dataset", "unknown")),
                        "action_type": str(case.get("action_type", "unknown")),
                        "valid": False,
...
class AgentLoopManagerTQ(AgentLoopManager):
    ...
    def evaluate_ersr_cases(
        self,
        cases: list[dict[str, Any]],
        settings: dict[str, Any],
    ) -> list[dict[str, Any]]:
        """Distribute ERSR cases across the existing AgentLoop workers."""

        if not cases:
            return []
        assignments = [list() for _ in self.agent_loop_workers]
        for index, case in enumerate(cases):
            assignments[index % len(assignments)].append(case)
        batches = ray.get(
            [
                worker.evaluate_ersr_cases.remote(assigned, settings)
                for worker, assigned in zip(
                    self.agent_loop_workers, assignments, strict=True
                )
                if assigned
            ]
        )
...
class PPOTrainer:
    ...
    def _validate(self) -> dict[str, float]:
        ...
        validation_response_lengths: list[float] = []
        validation_response_truncated: list[float] = []
        dump_all_inputs: list[str] = []
        dump_all_outputs: list[str] = []
        dump_all_keys: list[str] = []
        session_to_sample_idx: dict[str, int] = {}
        validation_rollout_records: list[dict[str, Any]] = []
        ...
        for batch_dict in self.val_dataloader:
            ...
            # Measure validation response lengths while the exact token tensor is
            # still available.  Reward functions run in separate Ray workers, so
            # custom reward_extra_info fields are not a reliable transport for
            # trainer-side response statistics.
            response_lengths = [int(tag["response_len"]) for tag in batch.tags]
            ...
            batch_uids = data.pop("uid").tolist()
            ...
            final_response_lengths = [float(response_lengths[i]) for i in final_indices]
            generation_config = self.config.get("rlvr_generation")
            validation_response_limit = (
                int(generation_config["val_max_new_tokens"])
                if generation_config is not None
                else int(self.config.data.max_response_length)
            )
            validation_response_lengths.extend(final_response_lengths)
            validation_response_truncated.extend(
                float(length >= validation_response_limit) for length in final_response_lengths
            )

            reward_model = data.pop("reward_model", None)
            if reward_model is not None:
                batch_gts = [item.get("ground_truth", None) for item in reward_model.tolist()]
                sample_gts.extend(batch_gts)
            else:
                batch_gts = [None] * len(final_indices)
                sample_gts.extend(batch_gts)

            data_source = data.pop("data_source", None)
            if data_source is not None:
                batch_data_sources = data_source.tolist()
                data_sources.extend(batch_data_sources)
            else:
                batch_data_sources = ["unknown"] * len(final_indices)
                data_sources.extend(batch_data_sources)

            # Preserve exact, unpadded validation token IDs for optional
            # step-level evaluations.  Never rebuild intervention boundaries
            # by decoding and re-tokenizing the response.
            for local_index, position in enumerate(final_indices):
                tag = batch.tags[position]
                prompt_length = int(tag["prompt_len"])
                response_length = int(tag["response_len"])
                prompt_row = text_data["prompts"][position]
                response_row = text_data["responses"][position]
                prompt_ids = (
                    prompt_row[-prompt_length:].tolist()
                    if prompt_length > 0
                    else []
                )
                response_ids = (
                    response_row[:response_length].tolist()
                    if response_length > 0
                    else []
                )
                dataset = str(batch_data_sources[local_index])
                correct = bool(float(scores[local_index]) > 0.5)
                if self._include_additional_validation_record(dataset, correct):
                    validation_rollout_records.append(
                        {
                            "uid": str(batch_uids[local_index]),
                            "dataset": dataset,
                            "answer": str(batch_gts[local_index]),
                            "correct": correct,
                            "prompt_ids": [int(value) for value in prompt_ids],
        ...
        reward_extra_infos_dict["response_length"] = validation_response_lengths
        ...
        metrics.update(
            self._additional_validation_metrics(validation_rollout_records)
        )
        return metrics

    def _additional_validation_metrics(
        self, validation_rollout_records: list[dict[str, Any]]
    ) -> dict[str, float]:
        """Hook for algorithm-specific evaluations that reuse validation rollouts."""
    ...
    def _include_additional_validation_record(
        ...
        """Return whether a rollout's exact IDs must survive normal validation."""
    ...
    def _compute_metrics(self, batch: KVBatchMeta, metrics, timing_raw, global_steps, epoch):
        ...
        # Per-trajectory durations make the two overlapped parts of the async
        # generation stage observable. The outer timing_s/gen remains the true
        # end-to-end wall clock for the combined pipeline.
        rollout_times = np.asarray(
            [float(item.get("generate_sequences", 0.0)) for item in agent_metrics], dtype=np.float64
        )[non_padding_mask]
        timing_series = {
            "student_rollout": rollout_times,
            "teacher_forward": np.asarray(
                [float(item.get("teacher_forward_s", 0.0)) for item in extra_fields], dtype=np.float64
            )[non_padding_mask],
            "teacher_engine": np.asarray(
                [float(item.get("teacher_engine_s", 0.0)) for item in extra_fields], dtype=np.float64
            )[non_padding_mask],
            "teacher_logprob_extract": np.asarray(
                [float(item.get("teacher_logprob_extract_s", 0.0)) for item in extra_fields], dtype=np.float64
            )[non_padding_mask],
            "teacher_queue_transfer": np.asarray(
                [float(item.get("teacher_queue_transfer_s", 0.0)) for item in extra_fields], dtype=np.float64
            )[non_padding_mask],
        }
        for name, values in timing_series.items():
            if values.size:
                metrics.update(
                    {
                        f"timing_s/{name}_mean": values.mean(),
                        f"timing_s/{name}_p95": np.percentile(values, 95),
                        f"timing_s/{name}_max": values.max(),
                        f"timing_s/{name}_sum": values.sum(),
                    }
                )

        for field_name in ("top16_overlap", "top16_mass_overlap"):
            values = np.asarray(
                [
                    float(item.get(field_name, 0.0) if item.get(field_name) is not None else 0.0)
                    for item in extra_fields
                ],
                dtype=np.float64,
            )[non_padding_mask]
            if values.size:
                metrics[f"opd/diagnostics/{field_name}"] = values.mean()
