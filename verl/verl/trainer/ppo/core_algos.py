...
@register_policy_loss("reinforce")  # type: ignore[arg-type]
def compute_policy_loss_reinforce_registered(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    """Exact ``-E[stopgrad(A) * log pi]`` objective used by PG-OPD.

    PG-OPD is explicitly on-policy and does not add importance weights to its
    mathematical objective. ``old_log_prob`` is retained only for the
    diagnostic rollout/current-policy KL reported by the shared helper.
    """
    del rollout_is_weights
    return compute_policy_loss_reinforce(
