...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Split response lengths by the binary rule-based outcome.  Sequence scores
    # are the unshaped reward-model scores (0 for incorrect, positive for
    # correct), so this remains independent of KL penalties and advantages.
    correct_mask = non_aborted_mask & (sequence_score > 0)
    incorrect_mask = non_aborted_mask & ~correct_mask
    correct_response_length = response_length[correct_mask]
    incorrect_response_length = response_length[incorrect_mask]
    outcome_length_metrics = {
        "response_length/correct_count": correct_response_length.numel(),
        "response_length/incorrect_count": incorrect_response_length.numel(),
    }
    # Omit an undefined mean instead of emitting NaN when a batch contains no
    # samples of one outcome; the corresponding count still makes that clear.
    if correct_response_length.numel() > 0:
        outcome_length_metrics["response_length/correct_mean"] = (
            torch.mean(correct_response_length).detach().item()
        )
    if incorrect_response_length.numel() > 0:
        outcome_length_metrics["response_length/incorrect_mean"] = (
